(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    [['dashboard','Home'],['customers','Customers'],['add','+'],['orders','Orders'],['more','More']].forEach(function (item) {
      var control = document.createElement(item[0] === 'add' || item[0] === 'more' ? 'button' : 'a');
      control.textContent = item[1];
      if (control.tagName === 'A') control.href = '?business=chemtech&page=' + item[0];
    ...
    [['Add customer','ct-customer-dialog'],['Create order','ct-order-dialog'],['Add product','ct-product-dialog'],['Generate invoice','ct-invoice-dialog'],['Record payment','ct-payment-dialog']].forEach(function(item){
      ...
      button.disabled=!target || !!target.querySelector('button[type="submit"]:disabled, button[name]:disabled');
      button.onclick=function(){ sheet.close(); target.showModal(); }; actions.appendChild(button);
    });
