...
$ctSelectedInvoiceLines = [];
foreach ($ctSelectedOrders as $selectedOrder) foreach (chemtechOrderItems($selectedOrder) as $selectedItem) $ctSelectedInvoiceLines[] = array_merge($selectedItem, ['order_number'=>$selectedOrder['order_number'] ?? '']);
...
foreach ($chemtechInvoices as $invoice) {
    ...
    $invoiceLines = [];
    foreach ($invoiceOrders as $invoiceOrder) foreach (chemtechOrderItems($invoiceOrder) as $invoiceItem) $invoiceLines[] = array_merge($invoiceItem, ['order_number'=>$invoiceOrder['order_number'] ?? '']);
    ...
    $ctInvoiceRecords[$invoiceId] = [
        ...
        'lines'=>array_map(static fn(array $item): array => [
            'description'=>$item['product_name'] ?? '', 'order_number'=>$item['order_number'] ?? '', 'hsn'=>$item['hsn'] ?? '',
            'quantity'=>(int) ($item['quantity_milli'] ?? 0) / 1000, 'unit'=>$item['unit'] ?? '', 'rate_paise'=>(int) ($item['unit_price_paise'] ?? 0),
            'subtotal_paise'=>(int) ($item['subtotal_paise'] ?? 0), 'tax_paise'=>(int) ($item['cgst_paise'] ?? 0)+(int) ($item['sgst_paise'] ?? 0)+(int) ($item['igst_paise'] ?? 0),
            'tax_rate_bps'=>(int) ($item['gst_rate_bps'] ?? 0), 'total_paise'=>(int) ($item['total_paise'] ?? 0),
        ], $invoiceLines),
        'totals'=>[
...
$ctOrderRecords = [];
foreach ($chemtechOrders as $order) $ctOrderRecords[(string) ($order['id'] ?? '')] = ['order_number'=>$order['order_number'] ?? '', 'items'=>chemtechOrderItems($order)];
?>
<div class="ct-workspace">
    ...
    <main class="ct-main">
        ...
        <?php elseif($chemtechPage==='orders'): ?>
            <section class="ct-card ct-index">
                <header class="ct-index-toolbar"><div><h2>Sales orders</h2><span><?= count($chemtechOrders) ?> total · one or multiple products per order</span></div><div class="ct-toolbar-actions"><label class="ct-search"><span>Search</span><input type="search" placeholder="Search orders" data-ct-table-filter="ct-order-table"></label><button class="ct-button primary" type="button" data-dialog-open="ct-order-dialog">Create order</button></div></header>
                <?php if(!$chemtechOrders): ?><div class="ct-empty"><b>No sales orders</b><span>Orders will appear here after creation.</span><button class="ct-button" type="button" data-dialog-open="ct-order-dialog">Create first order</button></div><?php else: ?>
                <div class="ct-table-wrap"><table class="ct-table" id="ct-order-table"><thead><tr><th>Order</th><th>Customer</th><th>Items</th><th>Pricing</th><th>Tax</th><th>Total</th><th>Billing</th><th></th></tr></thead><tbody>
                <?php foreach($chemtechOrders as $order): $orderItems=chemtechOrderItems($order); $itemNames=array_column($orderItems,'product_name'); ?><tr>
                    <td><b><?= $e($order['order_number']) ?></b><small><?= $e(date('d M Y',strtotime((string)$order['created_at']))) ?></small></td>
                    <td><?= $ctCustomerLink($order['customer_id'],$order['customer_name']) ?></td>
                    <td><b><?= count($orderItems)===1?$e($orderItems[0]['product_name']):count($orderItems).' products' ?></b><small><?= count($orderItems)===1?number_format($orderItems[0]['quantity_milli']/1000,3).' '.$e($orderItems[0]['unit']):$e(implode(', ',$itemNames)) ?></small></td>
                    <td><?= count($orderItems)===1?chemtechMoney((int)$orderItems[0]['unit_price_paise']):'Multiple prices' ?><small><?= count($orderItems)===1?'per '.$e($orderItems[0]['unit']):'Edit by product' ?></small></td>
                    <td><?= $e($order['tax_type']) ?></td><td><b><?= chemtechMoney((int)$order['total_paise']) ?></b></td><td><span class="ct-status <?= $e($order['billing_status']) ?>"><?= $e(ucfirst($order['billing_status'])) ?></span></td>
                    <td><?php if(($order['billing_status']??'unbilled')==='unbilled'): ?><button class="ct-table-action" type="button" data-ct-edit-order data-order-id="<?= $e($order['id']) ?>" data-order-number="<?= $e($order['order_number']) ?>">Edit price</button><?php elseif(!empty($order['invoice_id'])): ?><?= $ctInvoiceLink($order['invoice_id']) ?><?php else: ?><span class="ct-locked">Locked</span><?php endif; ?></td>
        ...
        <?php elseif($chemtechPage==='invoices'): ?>
            <?php if($ctSelectedInvoice): ?>
                ...
                $ctInvoiceDownloadData = [
                    ...
                    'lines'=>array_map(static fn(array $item):array=>['description'=>$item['product_name'],'order_number'=>$item['order_number'],'hsn'=>$item['hsn'],'quantity'=>$item['quantity_milli']/1000,'unit'=>$item['unit'],'rate_paise'=>$item['unit_price_paise'],'subtotal_paise'=>$item['subtotal_paise'],'tax_paise'=>(int)$item['cgst_paise']+(int)$item['sgst_paise']+(int)$item['igst_paise'],'tax_rate_bps'=>$item['gst_rate_bps'],'total_paise'=>$item['total_paise']],$ctSelectedInvoiceLines),
                ...
                <article class="ct-invoice-document" aria-label="GST invoice <?= $e($ctSelectedInvoice['invoice_number']) ?>">
                    ...
                    <div class="ct-table-wrap"><table class="ct-invoice-lines"><thead><tr><th>#</th><th>Description</th><th>HSN</th><th>Qty</th><th>Rate</th><th>Taxable</th><th>Tax</th><th>Total</th></tr></thead><tbody><?php foreach($ctSelectedInvoiceLines as $index=>$item): $lineTax=(int)$item['cgst_paise']+(int)$item['sgst_paise']+(int)$item['igst_paise']; ?><tr><td><?= $index+1 ?></td><td><b><?= $e($item['product_name']) ?></b><small><?= $e($item['order_number']) ?></small></td><td><?= $e($item['hsn'] ?: '—') ?></td><td><?= number_format($item['quantity_milli']/1000,3).' '.$e($item['unit']) ?></td><td><?= chemtechMoney((int)$item['unit_price_paise']) ?></td><td><?= chemtechMoney((int)$item['subtotal_paise']) ?></td><td><?= chemtechMoney($lineTax) ?><small><?= number_format($item['gst_rate_bps']/100,2) ?>%</small></td><td><b><?= chemtechMoney((int)$item['total_paise']) ?></b></td></tr><?php endforeach; ?></tbody></table></div>
...
<script type="application/json" id="ct-order-records"><?= json_encode($ctOrderRecords,JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_UNESCAPED_SLASHES) ?></script>
...
<dialog class="ct-dialog" id="ct-order-dialog"><form class="ct-dialog-panel ct-dialog-panel-wide" method="post" data-ct-order-form><header><div><span>Orders</span><h2>Create sales order</h2></div><button type="button" data-dialog-close aria-label="Close">×</button></header><div class="ct-dialog-body"><input type="hidden" name="csrf" value="<?= $e($_SESSION['csrf']) ?>">
    <fieldset class="ct-choice-group"><legend>Customer type</legend><label><input type="radio" name="customer_mode" value="existing" checked><span>Existing customer</span></label><label><input type="radio" name="customer_mode" value="one_time"><span>One-time buyer</span></label></fieldset>
    <section data-ct-customer-mode="existing"><label>Customer<select name="customer_id" required><option value="">Choose customer…</option><?php foreach($chemtechCustomers as $customer): ?><option value="<?= $e($customer['id']) ?>"><?= $e($customer['name'].' · state '.$customer['state_code']) ?></option><?php endforeach; ?></select></label></section>
    <section data-ct-customer-mode="one_time" hidden><div class="ct-form-grid three"><label>Buyer name<input name="one_time_name" placeholder="Walk-in customer"></label><label>Phone <small>optional</small><input name="one_time_phone" inputmode="tel"></label><label>State code <small>optional</small><input name="one_time_state_code" inputmode="numeric" maxlength="2" placeholder="<?= $e($chemtechSettings['state_code']) ?>"></label></div><p class="ct-field-help">A lightweight customer record is created automatically so the order and invoice stay connected.</p></section>
    <div class="ct-order-lines-head"><div><b>Products</b><span>Add every product included in this order.</span></div><button class="ct-button" type="button" data-ct-add-order-item>+ Add product</button></div>
    <div class="ct-order-lines" data-ct-order-items>
        <section class="ct-order-line" data-ct-order-item><label>Product<select name="items[0][product_id]" data-ct-order-product required><option value="">Choose product…</option><?php foreach($chemtechProducts as $product): ?><option value="<?= $e($product['id']) ?>" data-price="<?= number_format($product['sale_price_paise']/100,2,'.','') ?>" data-unit="<?= $e($product['unit']) ?>"><?= $e($product['name']) ?> · <?= number_format($product['stock_milli']/1000,3) ?> <?= $e($product['unit']) ?></option><?php endforeach; ?></select></label><label>Quantity<input type="number" name="items[0][quantity]" min=".001" step=".001" required></label><label>Unit price<input type="number" name="items[0][unit_price]" min=".01" step=".01" data-ct-order-price required></label><button type="button" class="ct-line-remove" data-ct-remove-order-item aria-label="Remove product" hidden>×</button></section>
    </div>
    <div class="ct-inline-note">Each product keeps its own quantity, price and GST rate. Stock reduces only after the complete order is validated.</div>
</div><footer><button class="ct-button" type="button" data-dialog-close>Cancel</button><button class="ct-button primary" name="create_chemtech_order" value="1">Create order</button></footer></form></dialog>

<template id="ct-order-item-template"><section class="ct-order-line" data-ct-order-item><label>Product<select name="items[__INDEX__][product_id]" data-ct-order-product required><option value="">Choose product…</option><?php foreach($chemtechProducts as $product): ?><option value="<?= $e($product['id']) ?>" data-price="<?= number_format($product['sale_price_paise']/100,2,'.','') ?>" data-unit="<?= $e($product['unit']) ?>"><?= $e($product['name']) ?> · <?= number_format($product['stock_milli']/1000,3) ?> <?= $e($product['unit']) ?></option><?php endforeach; ?></select></label><label>Quantity<input type="number" name="items[__INDEX__][quantity]" min=".001" step=".001" required></label><label>Unit price<input type="number" name="items[__INDEX__][unit_price]" min=".01" step=".01" data-ct-order-price required></label><button type="button" class="ct-line-remove" data-ct-remove-order-item aria-label="Remove product">×</button></section></template>

<dialog class="ct-dialog" id="ct-edit-order-dialog"><form class="ct-dialog-panel" method="post"><header><div><span>UNBILLED ORDER</span><h2>Edit order prices</h2></div><button type="button" data-dialog-close aria-label="Close">×</button></header><div class="ct-dialog-body"><input type="hidden" name="csrf" value="<?= $e($_SESSION['csrf']) ?>"><input type="hidden" name="order_id" data-ct-edit-order-id><div class="ct-inline-note"><b data-ct-edit-order-number>Order</b><br>Taxable value, GST and the final total recalculate automatically.</div><div class="ct-edit-price-lines" data-ct-edit-order-items></div><p class="ct-field-help">Invoiced orders remain locked to protect issued invoices.</p></div><footer><button class="ct-button" type="button" data-dialog-close>Cancel</button><button class="ct-button primary" name="update_chemtech_order_price" value="1">Update prices</button></footer></form></dialog>
